...
int main()
{
   std::string filename = "science.txt";
   ...
   printf("ServerS is up and running using UDP on port %s.\n", SERVER_S_PORT);
   ...
   while (1)
   {
      ...
      if (username == "firns")
      {
         std::cout << "ServerS received an inventory status request for code " << bookCode << "." << std::endl;
      }
      else
      {
         std::cout << "ServerS received " << bookCode << " code from the Main Server." << std::endl;
      }
      int response = -1;
      ...
      if (username == "firns")
      {
         std::cout << "ServerS finished sending the inventory status to the Main server using UDP on port " << SERVER_S_PORT << "." << std::endl;
      }
      else
      {
         std::cout << "ServerS finished sending the availability status of code " << bookCode << " to the Main Server using UDP on port " << SERVER_S_PORT << "." << std::endl;
      }
   }
