...
int main()
{
   ...
   if ((rv = getaddrinfo(HOST_NAME, SERVER_H_PORT, &hints, &servinfo)) != 0)
   {
   ...
   while (1)
   {
      ...
      if (username == "firns")
      {
         std::cout << "ServerH received an inventory status request for code " << bookCode << "." << std::endl;
      }
      else
      {
         std::cout << "ServerH received " << bookCode << " code from the Main Server." << std::endl;
      }
      int response = -1;
      ...
      if (username == "firns")
      {
         std::cout << "ServerH finished sending the inventory status to the Main server using UDP on port " << SERVER_H_PORT << "." << std::endl;
      }
      else
      {
         std::cout << "ServerH finished sending the availability status of code " << bookCode << " to the Main Server using UDP on port " << SERVER_H_PORT << "." << std::endl;
      }
   }
