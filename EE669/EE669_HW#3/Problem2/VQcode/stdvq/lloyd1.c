...

{
  ...
  olddistortion = HUGE_VAL; /* first pass requires very large distortion */
  ...
  do {
	...
	tempdouble += temp*temp;
	 ...
	 norm(input)-distance <= norm(codeword) <= norm(input)+distance */
      ...
      bestdistortion = HUGE_VAL; /* keep convention that ties go to lower index */
